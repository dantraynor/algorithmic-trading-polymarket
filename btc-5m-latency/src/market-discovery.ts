...
export class MarketDiscovery {
  ...
  /**
   * Fetch the resolved outcome from Gamma API.
   * Returns 'UP', 'DOWN', or null if not yet resolved.
   */
  async fetchResolvedOutcome(windowTimestamp: number): Promise<'UP' | 'DOWN' | null> {
    const slug = `btc-updown-5m-${windowTimestamp}`;
    ...
    try {
      ...
      if (outcomePrices.length < 2 || outcomes.length < 2) return null;

      // Check if resolved (one side should be "1", the other "0")
      const winnerIndex = outcomePrices.findIndex((p: string) => p === '1');
      if (winnerIndex < 0) return null; // Not yet resolved

      const winnerLabel = outcomes[winnerIndex]?.toLowerCase();
      if (winnerLabel === 'up' || winnerLabel === 'yes') return 'UP';
      if (winnerLabel === 'down' || winnerLabel === 'no') return 'DOWN';

      logger.warn('Unexpected winner label from Gamma API', { slug, winnerLabel, outcomes, outcomePrices });
      return null;
    } catch (error: any) {
      logger.warn('Failed to fetch resolved outcome from Gamma API', { slug, error: error.message });
      return null;
    }
  }
}
