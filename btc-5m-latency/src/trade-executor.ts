/**
 * Trade Executor - Places single-side orders on Polymarket CLOB.
 * Adapted from btc-5m-momentum: same EIP-712 signing, uses generic TradeOrder input.
 */
...
import { Config, TradeOrder, SingleTradeResult, OrderEIP712, OrderResponse } from './types';
...
export class TradeExecutor {
  ...
  constructor(config: Config, redis: Redis, orderbookChecker?: OrderbookChecker) {
    ...
    // BTC 5-min markets are NOT negRisk — use the standard CTF Exchange
    this.domain = {
      name: 'Polymarket CTF Exchange',
      version: '1',
      chainId: config.chainId,
      verifyingContract: config.ctfExchangeAddress || '0x4bFb41d5B3570DeFd03C39a9A4D8dE6Bd8B8982E',
    };
    ...
    // Error interceptor — log CLOB API response body on failures
    this.client.interceptors.response.use(
      (response) => response,
      (error) => {
        if (error.response) {
          logger.error('CLOB API error response', {
  ...
  /**
   * Execute a single-side trade.
   */
  async executeSingle(order: TradeOrder): Promise<SingleTradeResult> {
    const startTime = performance.now();

    logger.info('Executing latency trade', {
      side: order.side,
      limitPrice: order.limitPrice.toFixed(4),
      shares: order.shares.toFixed(2),
    });

    if (this.config.dryRun) {
      if (!this.paperSimulator) {
        logger.error('Dry-run mode but PaperSimulator not initialized');
        return {
          success: false,
      ...
      const simResult = await this.paperSimulator.simulateFill(order);
      return {
    ...
    if (order.shares.lte(maxChunk)) {
      ...
      try {
        ...
        const orderIds = result.orderID ? [result.orderID] : undefined;
        ...
        return {
          ...
          side: order.side,
          price: order.limitPrice,
          size: result.success ? order.shares : new Decimal(0),
          error: result.errorMsg,
          latencyMs: elapsed,
        };
      } catch (error: any) {
        const elapsed = performance.now() - startTime;
        logger.error(`${order.side} order failed:`, { error: error.message });
        return {
          success: false,
          tokenId: order.tokenId,
          side: order.side,
    ...
    return this.executeChunked(order, maxChunk, startTime);
  }
  ...
  private async executeChunked(
    ...
    logger.info(`Order split into ${chunks.length} chunks`, {
      totalShares: order.shares.toFixed(2),
    ...
    try {
      ...
      return {
        ...
        orderId: orderIds.length > 0 ? orderIds[orderIds.length - 1] : undefined,
  ...
  private async signAndSubmitOrder(
    ...
    logger.info('Order signing details', {
      domain: this.domain,
