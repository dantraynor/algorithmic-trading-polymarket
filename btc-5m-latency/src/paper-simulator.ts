...
import { TradeOrder, SimulatedTradeResult } from './types';
...
export class PaperSimulator {
  ...
  async simulateFill(order: TradeOrder): Promise<SimulatedTradeResult> {
    const startTime = performance.now();

    // Re-fetch fresh book to simulate latency drift
    const book = await this.orderbookChecker.fetchOrderBook(order.tokenId);
    const asks = book.asks;

    // Empty book -> missed fill
    if (asks.length === 0) {
      return this.missedResult(order, performance.now() - startTime);
    }
    ...
    if (!liquidity) {
      return this.missedResult(order, performance.now() - startTime, bestAskPrice);
    }

    const fillShares = liquidity.availableShares;
    const fillPrice = liquidity.vwapPrice;
    const partialFill = fillShares.lt(order.shares);
    const fillRatio = fillShares.div(order.shares).toNumber();
    ...
    for (const level of asks) {
      ...
      if (price.gt(order.limitPrice)) break;
      levelsConsumed++;
    ...
    logger.info('[PAPER] Fill simulation', {
      side: order.side,
      requested: order.shares.toFixed(2),
